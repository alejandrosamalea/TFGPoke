...
public class AtaqueService {
    ...
    public Ataque findByNombre(String nombreAtaque) {
        return ataqueRepository.findByNombre(nombreAtaque);
    }

    public Ataque obtenerAtaqueAleatorio (Long idTipo) {
        List<Ataque> ataques = ataqueRepository.findAll();
        Random random = new Random();
        int randomIndex = random.nextInt(ataques.size());
        Ataque ataque = ataques.get(randomIndex);
        while (ataque.getTipo().getId() != idTipo) {
            randomIndex = random.nextInt(ataques.size());
            ataque = ataques.get(randomIndex);
        }
        return ataque;
    }
